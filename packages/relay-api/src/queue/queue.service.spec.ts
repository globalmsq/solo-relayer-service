...
describe('QueueService', () => {
  ...
  describe('sendDirectTransaction', () => {
    ...
    it('should create transaction and send to SQS successfully', async () => {
      ...
      jest.spyOn(prismaService.transaction, 'create').mockResolvedValueOnce(mockTransaction as any);
    ...
    it('should rollback transaction to failed status when SQS send fails', async () => {
      ...
      jest.spyOn(prismaService.transaction, 'update').mockResolvedValueOnce({} as any);
    ...
    it('should throw ServiceUnavailableException when DB creation fails', async () => {
      ...
      jest.spyOn(prismaService.transaction, 'create').mockRejectedValueOnce(dbError);
  ...
  describe('sendGaslessTransaction', () => {
    ...
    it('should create transaction and send to SQS successfully', async () => {
      ...
      // Act
      const result = await service.sendGaslessTransaction(gaslessDto, forwarderAddress);
    ...
    it('should rollback transaction to failed status when SQS send fails', async () => {
      ...
      // Act & Assert
      await expect(service.sendGaslessTransaction(gaslessDto, forwarderAddress)).rejects.toThrow(
        ServiceUnavailableException,
      );
